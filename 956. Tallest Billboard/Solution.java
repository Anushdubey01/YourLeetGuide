    ...
    public int tallestBillboard(int[] rods) {
        ...
        for (int h : rods) {
            ...
            for (int diff : curr.keySet()) {
                ...
                // Don't use the current rod
                dp.put(diff, Math.max(dp.getOrDefault(diff, 0), height));

                // Put current rod on taller pile
                dp.put(diff + h, Math.max(dp.getOrDefault(diff + h, 0), height));

                // Put current rod on shorter pile
                dp.put(Math.abs(diff - h), Math.max(dp.getOrDefault(Math.abs(diff - h), 0), height + Math.min(diff, h)));
            }
        }
